<%-- 
    Document   : employeePassengerInfo
    Created on : Apr 25, 2017, 8:34:06 AM
    ...
    if(request.getParameter("submitted") != null && request.getParameter("submitted").equals("true")){
        ...
        if(flag1 || flag2 || flag3 || flag4){
            ...
            if(request.getParameter("email") != null){
                ...
                if(db.emailAlreadyUsed(userEmail)){
                    String userID = db.selectString("id", "userr", "email", userEmail);
                
                    session.setAttribute("passenger_user_id", userID);
                    response.sendRedirect("/IowaAir/purchaseFlight.jsp");
                    return;
                } else {
                
                    String firstName = request.getParameter("firstName0");
                    String lastName = request.getParameter("lastName0");
                    
                    SendMail mailer = new SendMail(userEmail);
                    
                    String validationCode = mailer.send(false);

                    //add user to database
                    db.addUserToDatabase(firstName, lastName,
                        userEmail, User_Types.customer, null, null, validationCode, validationCode);
                    
                    mailer.send(true, validationCode);
                    flag5=true;
...
<html>
    ...
    <body>
        ...
        <div class="middle">
           ...
           <h1>New user successfully added to database using email provided.  User's password and confirmation code has been sent to this email.</h1>
           <form action="purchaseFlight.jsp" method="POST">
               <input type="submit" value="Continue to Book Flight"><br>
           </form>
